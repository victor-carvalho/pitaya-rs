use super::{Discovery, Error, Notification};
use crate::{settings, Server, ServerId, ServerKind};
...
impl EtcdLazy {
    ...
    async fn cache_server_kind(&mut self, server_kind: &ServerKind) -> Result<(), Error> {
        ...
        for kv in resp.kvs() {
            ...
            let new_server: Arc<Server> =
                Arc::new(serde_json::from_str(server_str).map_err(Error::CorruptServer)?);
